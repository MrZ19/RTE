...
from util import visualize_trans_result
...
def _init_(args):
    if not os.path.exists('../../../../home1/zhangzhiyuan/checkpoints'):
        os.makedirs('../../../../home1/zhangzhiyuan/checkpoints')
    if not os.path.exists('../../../../home1/zhangzhiyuan/checkpoints/' + args.exp_name):
        os.makedirs('../../../../home1/zhangzhiyuan/checkpoints/' + args.exp_name)
    if not os.path.exists('../../../../home1/zhangzhiyuan/checkpoints/' + args.exp_name + '/' + 'models'):
        os.makedirs('../../../../home1/zhangzhiyuan/checkpoints/' + args.exp_name + '/' + 'models')
    os.system('cp main.py ../../../../home1/zhangzhiyuan/checkpoints' + '/' + args.exp_name + '/' + 'main.py.backup')
    os.system('cp model.py ../../../../home1/zhangzhiyuan/checkpoints' + '/' + args.exp_name + '/' + 'model.py.backup')
    os.system('cp data.py ../../../../home1/zhangzhiyuan/checkpoints' + '/' + args.exp_name + '/' + 'data.py.backup')
...
def test_one_epoch(args, net, test_loader):
    ...
    for src, target, rotation_ab, translation_ab, rotation_ba, translation_ba, euler_ab, euler_ba in tqdm(test_loader):
        ...
        rotation_ab_pred, translation_ab_pred, _, _, rotation_ba_pred, translation_ba_pred = net(src, target)
        ...
        #visualize_trans_result(src, transformed_src, target, rotation_ab_pred, translation_ab_pred, rotation_ab, translation_ab)
...
def train_one_epoch(args, net, train_loader, opt):
    ...
    for src, target, rotation_ab, translation_ab, rotation_ba, translation_ba, euler_ab, euler_ba, pc_a, pc_b, R1, R2 in tqdm(train_loader):
        src = src.cuda()
        target = target.cuda()
        rotation_ab = rotation_ab.cuda()
        translation_ab = translation_ab.cuda()
        ...
        opt.zero_grad()
        num_examples += batch_size
        rotation_ab_pred, translation_ab_pred, point_ori_ab, point_cor_ab, rotation_ba_pred, translation_ba_pred = net(src, target)
        rotation_ab_pred_e, translation_ab_pred_e, point_ori_ab_e, point_cor_ab_e, _, _ = net(pc_a, pc_b)
        ...
        transformed_target = transform_point_cloud(target, rotation_ba_pred, translation_ba_pred)
        ###########################
        #print("point shape: {}".format(point_ori_ab.shape))
        point_a = point_ori_ab.transpose(2,1).contiguous().view(-1,3)
        point_b = point_cor_ab.transpose(2,1).contiguous().view(-1,3)
        loss_distance1 = torch.mean(torch.norm((point_a - point_b),dim=1)).item()

        point_a_e = point_ori_ab_e.transpose(2,1).contiguous().view(-1,3)
        point_b_e = point_cor_ab_e.transpose(2,1).contiguous().view(-1,3)
        loss_distance2 = torch.mean(torch.norm((point_a_e - point_b_e),dim=1)).item()
        ###########################
        identity = torch.eye(3).cuda().unsqueeze(0).repeat(batch_size, 1, 1)
        R_e = torch.matmul(torch.matmul(R2.transpose(2,1), rotation_ab_pred_e), R1)
        #print("R2T shape:{},tabpe shape:{}".format(R2.shape,translation_ab_pred_e.shape))
        t_e = torch.matmul(R2.transpose(2,1), translation_ab_pred_e.unsqueeze(2)).squeeze()
        loss2 = F.mse_loss(torch.matmul(R_e.transpose(2, 1), rotation_ab), identity) \
               + F.mse_loss(t_e, translation_ab) 
        loss1 = F.mse_loss(torch.matmul(rotation_ab_pred.transpose(2, 1), rotation_ab), identity) \
               + F.mse_loss(translation_ab_pred, translation_ab)
        ###########################
        #R_left = (torch.matmul(R2, torch.matmul(rotation_ab_pred, src) + translation_ab_pred.unsqueeze(2))).squeeze()
        #R_right = torch.matmul(torch.matmul(rotation_ab_pred_e, R1), src) + translation_ab_pred_e.unsqueeze(2)
        #loss3 = F.mse_loss(R_left, R_right)
        R_left = torch.matmul(R2, rotation_ab_pred)
        R_right = torch.matmul(rotation_ab_pred_e, R1)
        loss3 = F.mse_loss(R_left, R_right)
        t_left = torch.matmul(R2, translation_ab_pred.unsqueeze(2)).squeeze(2)
        t_right = translation_ab_pred_e
        loss4 = F.mse_loss(t_left, t_right)
        loss5 = F.mse_loss(torch.matmul(R_left.transpose(2,1),R_right), identity) + F.mse_loss(t_left, t_right)
        ###########################
        loss = loss1 + loss2 + loss5 #loss3+loss4 #+ loss_distance1 + loss_distance2
        if args.cycle:
            rotation_loss = F.mse_loss(torch.matmul(rotation_ba_pred, rotation_ab_pred), identity.clone())
            translation_loss = torch.mean((torch.matmul(rotation_ba_pred.transpose(2, 1),
                                                        translation_ab_pred.view(batch_size, 3, 1)).view(batch_size, 3)
    ...
    for epoch in range(args.epochs):
        ...
        if best_test_loss >= test_loss:
            ...
            if torch.cuda.device_count() > 1:
                ...
                #torch.save(net.module.state_dict(), '../../../../home1/zhangzhiyuan/checkpoints/%s/models/model.%d.t7' % (args.exp_name, epoch))
                torch.save(net.module.state_dict(), '../../../../home1/zhangzhiyuan/checkpoints/%s/models/model.best.t7' % args.exp_name)
            else:
                #torch.save(net.state_dict(), 'checkpoints/%s/models/model.best.t7' % args.exp_name)
                #torch.save(net.state_dict(), '../../../../home1/zhangzhiyuan/checkpoints/%s/models/model.%d.t7' % (args.exp_name, epoch))
                torch.save(net.state_dict(), '../../../../home1/zhangzhiyuan/checkpoints/%s/models/model.best.t7' % args.exp_name)
...
def main():
    ...
    parser.add_argument('--num_points', type=int, default=256, metavar='N',
    ...
    boardio = SummaryWriter(log_dir='../../../../home1/zhangzhiyuan/checkpoints/' + args.exp_name)
    _init_(args)

    #textio = IOStream('checkpoints/' + args.exp_name + '/run.log')
    textio = IOStream('../../../../home1/zhangzhiyuan/checkpoints/' + args.exp_name + '/run.log')
